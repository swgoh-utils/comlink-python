...
def _format_stats(stats: dict, level: int = 85, options: dict = None) -> dict:
    ...
    if stats['mods']:
        logger.debug(f"{utils.get_function_name()}: Rounding mod values ... ")
        for stat_id, value in stats['mods'].items():
            stats['mods'][stat_id] = round(value, 6)
            logger.debug(f"{utils.get_function_name()}: {stat_id} old value: {value}, "
                         + f"new value: {stats['mods'][stat_id]}")

    if scale != 1:
        logger.debug(f"{utils.get_function_name()}: Scaling stats ... {scale=}")
        for stat_type in ('base', 'gear', 'crew', 'growthModifiers', 'mods'):
            if stat_type in stats:
                for stat_id in stats[stat_type].keys():
                    ...
                    logger.debug(f"{utils.get_function_name()}: {stat_id} old value: {original_value}, "
                                 + f"new value: {stats[stat_type][stat_id]}")
            else:
                logger.debug(f"{utils.get_function_name()}: {stat_type} not found in {list(stats.keys())}")
    ...
    if options.get('percentVals') or options.get('gameStyle'):  # 'gameStyle' flag inherently includes 'percentVals'
        def convert_percent(stat_id_str: str, convert_func: Callable):
            logger.debug(f"{utils.get_function_name()}: {stat_id_str=}, {convert_func=}")
            flat = stats['base'].get(stat_id_str, 0)
            percent = convert_func(flat)
            stats['base'][stat_id_str] = percent
            last = percent
            logger.debug(f"{utils.get_function_name()}: {flat=}, {percent=}, {last=}")
    ...
    if options.get("gameStyle"):
        ...
        logger.debug(f"{utils.get_function_name()}: {stat_list=}")

        def add_stats(stat_id_str: str):
            logger.debug(f"{utils.get_function_name()}: {stat_id_str=} ({type(stat_id_str)})...")
            if not isinstance(stat_id_str, str):
                logger.debug(f"{utils.get_function_name()}: converting 'stat_id' from {type(stat_id_str)} to 'str' ...")
                stat_id_str = str(stat_id_str)
            if stat_id_str not in stat_list:
                logger.debug(f"{utils.get_function_name()}: {stat_id_str} not in stat_list. Adding it ...")
                stat_list.append(stat_id_str)

        if "gear" in stats:  # Character
            logger.debug(f"{utils.get_function_name()}: Processing 'gear' ...")
            for stat_id in stats['gear']:
                add_stats(stat_id)  # add stats from gear to list
            if 'mods' in stats:
                logger.debug(f"{utils.get_function_name()}: Processing 'mods' ...")
            ...
            for stat_id in stat_list:
                ...
                logger.debug(f"{utils.get_function_name()}: {flat_stat_id=} ({type(flat_stat_id)})... ...")
...
def _convert_flat_def_to_percent(
    ...
    logger.debug(f"*** Converting flat defense value to percent ({value=}, {level=}, {scale=}, {is_ship=}) *** ")
    val = value / scale
...
class StatCalc:
    ...
    def _calculate_base_stats(cls, stats: dict, level: int, base_id: str) -> dict:
        ...
        for i in ("2", "3", "4"):
            ...
            logger.info(f"Adding math_floor({stats['growthModifiers'][i]} * {level} * 1e8) / 1e8")
        ...
        logger.info(f"{utils.get_function_name()}: Calculating effect of primary stat on secondary stats ...")
        logger.debug(f"{utils.get_function_name()}: {stats['base']=}")
        ...
        logger.info(f"{utils.get_function_name()}: Ensuring core stats are present and at minimum ...")
        ...
        logger.debug(f"{utils.get_function_name()}: Base stat calculated: {stats['base']=}")
        return stats

    @classmethod
    def _calculate_mod_stats(cls, base_stats: dict, char: dict = None) -> dict or None:
        logger.info("{utils.get_function_name()}: Calculating mod stats ... ")
        logger.info(f"{utils.get_function_name()}: {base_stats=}")
        if not char.get('mods') and not char.get('equippedStatMod'):
            logger.warning(f"{utils.get_function_name()}: Mod list is missing or empty. Returning.")
            return char
        ...
        if char.get('equippedStatMod'):
            for mod in char['equippedStatMod']:
                ...
                mod_set_name = utils.Constants.MOD_SET_IDS[str(mod_set_id)]
                set_bonus = set_bonuses.get(mod_set_id)
                logger.debug(f"{utils.get_function_name()}: Mod set id: {mod_set_id} ({mod_set_name}), {set_bonus=}")
                if set_bonus:
                    # set bonus already found, increment
                    logger.debug(f"{utils.get_function_name()}: {mod_set_id} ({mod_set_name}) "
                                 + f"already present in set_bonuses list. Incrementing counter.")
                    set_bonus['count'] += 1
                    if mod['level'] == 15:
                        set_bonus['maxLevel'] += 1
                    logger.debug(f"{utils.get_function_name()}: {set_bonus['count']=}, "
                                 + f"{set_bonus['maxLevel']=}, {mod['level']=}")
                else:
                    # new set bonus, create object
                    logger.debug(f"{utils.get_function_name()}: Creating new entry for {mod_set_id} "
                ...
                i = 0
                logger.debug(f"{utils.get_function_name()}: counter = {i}, {stat=}")
                while True:
                    ...
                    logger.debug(f"{utils.get_function_name()}: raw_mod_stats[{stat['unitStatId']}] = {unscaled_stat}")
            ...
            logger.debug(f"{utils.get_function_name()}: Failed to find 'definitionId' in {char['defId']}. "
                         + f"Returning empty dictionary.")
            return {}

        # add stats given by set bonuses
        logger.debug(f"{utils.get_function_name()}: Adding mod set bonuses ...")
        for set_id, set_bonus in set_bonuses.items():
            ...
            logger.debug(f"{utils.get_function_name()}: {count=}, {max_count=}, {multiplier=}")
            raw_mod_stat = raw_mod_stats.get(set_def['id'], 0) + (set_def['value'] * multiplier)

            logger.debug(f"{utils.get_function_name()}: raw_mod_stats[{set_def['id']}] = {raw_mod_stat}")
            raw_mod_stats[set_def['id']] = raw_mod_stat

        # calculate actual stat bonuses from mods
        logger.debug(f"{utils.get_function_name()}: Calculating stat bonuses from mods ...")
        mod_stats = {}
        for stat_id, value in raw_mod_stats.items():
            ...
            if stat_id == 41:  # Offense
                ...
                logger.debug(f"{utils.get_function_name()}: {stat_id=} ({cls._STAT_NAME_MAP[str(stat_id)]}), "
    ...
    def _get_char_raw_stats(cls, char: dict) -> dict:
        ...
        logger.info(f"{utils.get_function_name()}: raw stats: {stats=}")
        # Calculate stats from current gear
        if len(char["equipment"]) != 0:
            logger.info(f"{utils.get_function_name()}: Calculating stats for {char['defId']} equipment ")
            for equipment_piece in char["equipment"]:
                equipmentId = equipment_piece["equipmentId"]
                logger.info(f"{utils.get_function_name()}: Searching for {equipmentId} in game gearData ...")
                if equipmentId not in cls._GEAR_DATA:
                    logger.info(f"{utils.get_function_name()}: {equipmentId=} not found in game gearData ...")
                    continue
            ...
            logger.info(f"{utils.get_function_name()}: No equipment for {char['defId']}")

        if char.get("relic") and char["relic"]["currentTier"] > 2:
            current_relic_tier: int = char["relic"].get("currentTier")
            logger.info(f"{utils.get_function_name()}: Calculating stats for {char['defId']} relic level: "
            ...
            for g_id in list(relic_tier_data["gms"].keys()):
                ...
                logger.info(f"{utils.get_function_name()}: {stats['growthModifiers'][g_id]=}")
        else:
            logger.info(f"{utils.get_function_name()}: No relic information for {char['defId']}")
        logger.info(f"{utils.get_function_name()}: Stat results for {char['defId']}: {stats}")
        return stats
    ...
    def calc_char_stats(
        ...
        logger.info(f"{utils.get_function_name()}: Calculating stats for {char['defId']}")

        if options is None:
            logger.debug(f"{utils.get_function_name()}: No options provided. Copying defaults ...")
    ...
    def calc_ship_stats(
        ...
        logger.info(f"{utils.get_function_name()}: Calculating stats for {ship['defId']}")
    ...
    def _set_skills(cls, unit_id: str, val: str) -> list:
        logger.info(f"{utils.get_function_name()}: Setting skills for {unit_id}")
        if val == "max":
            return [
    ...
    def _use_values_char(cls, char: dict, use_values: dict = None) -> dict:
        ...
        """
        logger.info(f"{utils.get_function_name()}: Executing _use_values_char() ...")
        ...
        if not use_values:
            logger.info(f"{utils.get_function_name()}: No 'use_values' argument provided. Returning.")
            return char
    ...
    def _calc_char_gp(cls, char: dict):
        logger.debug(f"{utils.get_function_name()}: *** Calculating character GP for {char['defId']} ***")

        gp = cls._GP_TABLES['unitLevelGP'][str(char['currentLevel'])]
        logger.debug(f"{utils.get_function_name()}: Char level: {str(char['currentLevel'])}, "
                     + f"GP added: {cls._GP_TABLES['unitLevelGP'][str(char['currentLevel'])]}")
        logger.debug(f"{utils.get_function_name()}: {gp=} after level adjustment "
        ...
        logger.debug(f"{utils.get_function_name()}: {gp=} after rarity adjustment")

        gp += cls._GP_TABLES['gearLevelGP'][str(char['currentTier'])]
        logger.debug(f"{utils.get_function_name()}: {gp=} after equipment level adjustment")
        ...
        if len(char['equipment']) > 0:
            logger.debug(f"{utils.get_function_name()}: *** Equipment found (processing) ***")
            for piece in char['equipment']:
                gp += cls._GP_TABLES['gearPieceGP'][str(char['currentTier'])][str(piece['slot'])]
        else:
            logger.debug(f"{utils.get_function_name()}: *** No equipment found (skipping) ***")
        ...
        if char.get('relic') and char['relic']['currentTier'] > 2:
            ...
            logger.debug(f"{utils.get_function_name()}: {gp=} after relic adjustment")
        logger.debug(f"{utils.get_function_name()}: Final GP: {math_floor(gp * 1.5)}")
    ...
    def _get_skill_gp(cls, def_id: str, skill: dict) -> float:
        logger.debug(f"{utils.get_function_name()}: *** Getting skill GP *** {def_id=}, {skill=}")
        for s in cls._UNIT_DATA[def_id]['skills']:
            if s['id'] == skill['id']:
                ...
                logger.debug(f"{utils.get_function_name()}: Original skill tier: {skill['tier']}, "
                             + f"Adjusted skill tier: {player_unit_skill_tier}")
                logger.debug(f"{utils.get_function_name()}: Found skill {skill['id']} "
                             + f"tier {player_unit_skill_tier} ... ")
                logger.debug(f"{utils.get_function_name()}: _GP_TABLES skill {s} ")
                if int(player_unit_skill_tier) > s['maxTier']:
                    temp_skill_tier = player_unit_skill_tier
                    player_unit_skill_tier = str(s['maxTier'])
                    logger.debug(f"{utils.get_function_name()}: Adjusting skill tier down to maximum.  "
                ...
                if power_override_tag:  # 'zeta' or 'omicron' or 'upgrade'
                    logger.debug(f"{utils.get_function_name()}: {power_override_tag=}, returning "
                ...
                # if no 'powerOverrideTags' exists for the player unit skill, use the GP for skill tier
                    ...
                    logger.debug(f"{utils.get_function_name()}: No 'powerOverrideTags' found. Using "
    ...
    def _process_language(cls, language: str = None) -> None:
        ...
        logger.info(f"{utils.get_function_name()}: Processing language: {language}")
        languages: list = DataBuilder.get_languages()
        if language is not None and language in languages:
            cls._LANGUAGE = language
        else:
            logger.warning(f"{utils.get_function_name()}: Unknown language: {language}, "
    ...
    def _process_options(cls, options: list or dict, set_global: bool = False) -> dict:
        ...
        logger.info(f"{utils.get_function_name()}: Processing options: type({type(options)})")
        new_options = cls._OPTIONS.copy()
        if isinstance(options, list):
            for option in options:
                if option in cls._ALLOWED_OPTIONS:
                    logger.info(f"{utils.get_function_name()}: Setting cls._OPTIONS['{option}'] to True.")
                    new_options[option] = True
                    if set_global:
                        cls._OPTIONS[option] = True
                else:
                    logger.warning(f"{utils.get_function_name()}: Unknown option: {option}")
        elif isinstance(options, dict):
            for option in list(options.keys()):
                if option in cls._ALLOWED_OPTIONS:
                    logger.info(f"{utils.get_function_name()}: Setting cls._OPTIONS[{option}] to {options[option]}.")
            ...
            err_msg = (f"{utils.get_function_name()}: Invalid options argument: expected type list() or dict(), "
                       + f"got {type(options)}")
            logger.exception(err_msg)
            raise StatCalcRuntimeError(err_msg)
        logger.info(f"{utils.get_function_name()}: Returning {new_options=}")
        return new_options
    ...
    def _process_use_values_settings(cls, use_values: dict) -> None:
        ...
        if not use_values:
            logger.warning(f"{utils.get_function_name()}: No 'use_values' parameter. Returning ...")
            return
        if not isinstance(use_values, dict):
            raise StatCalcRuntimeError(
                (f"{utils.get_function_name()}: Invalid object type ({type(use_values)}) for "
                 + f"'options' attribute. Should be dict.")
            )
        for key in list(use_values.keys()):
            if key not in ["char", "ship", "crew"]:
                logger.debug(f"{utils.get_function_name()}: Deleting unexpected element: {key}")
                del use_values[key]
    ...
    def calc_player_stats(
        ...
        """
        if not players:
            logger.error(f"{utils.get_function_name()}: No player rosters submitted to calc_player_stats(). Exiting...")
            return

        if isinstance(players, list):
            for player in players:
                logger.info(f"{utils.get_function_name()}: Processing player: {player['name']} ...")
                player['rosterUnit'] = cls.calc_roster_stats(player['rosterUnit'], options, use_values)
        else:
            # Single player roster
            logger.info(f"{utils.get_function_name()}: Processing player: {players['name']} ...")
            if 'rosterUnit' in players.keys():
                players['rosterUnit'] = cls.calc_roster_stats(players['rosterUnit'], options, use_values)
            else:
                logger.warning(f"{utils.get_function_name()}: No unit roster detected in submission. Exiting.")
                return
        return players
    ...
    def calc_roster_stats(
        ...
        :rtype: list
        """
        logger.info(f"{utils.get_function_name()}: *** Entering calc_roster_stats() ***")
        if not cls.is_initialized:
            raise StatCalcException(
                "StatCalc is not initialized. Please perform the initialization first."
            )
        ...
        if isinstance(units, list):
            ...
            for unit in units:
                ...
                if not unit or not cls._UNIT_DATA[defId]:
                    logger.warning(f"{utils.get_function_name()}: Unable to find {defId} in game data.")
                    return []
            ...
            raise StatCalcRuntimeError(
                f"{utils.get_function_name()}: Unsupported data type [{type(units)}] for 'unit' parameter"
            )
    ...
    def initialize(cls, cl: swgoh_comlink.SwgohComlink, language: str = "eng_us", **kwargs) -> bool:
        ...
        if language not in DataBuilder.get_languages():
            logger.warning(f"{utils.get_function_name()}: Provided language {language} is not supported. "
                           + f"Defaulting to 'eng_us'")
            setattr(cls, "language", "eng_us")
        else:
            logger.debug(f"{utils.get_function_name()}: Setting StatCalc.language to {language}")
        ...
        logger.info(f"{utils.get_function_name()}: Initializing StatCalc for first time use.")
        # Populate the _OPTIONS dictionary with default values
        cls.reset_options()

        cls._COMLINK = cl
        class_vars = vars(cls)
        logger.debug(f"{utils.get_function_name()}: Class vars: {class_vars.keys()}")
        for param, value in kwargs.items():
            if param in allowed_parameters.keys():
                if value in allowed_parameters[param]:
                    if param == 'debug':
                        Constants.set_debug(value)
                        continue
                    class_var = "_" + param.upper()
                    logger.debug(f"{utils.get_function_name()}: Setting class variable {class_var} to {value}")
                    # Remove .json file extension from file name arguments since it is added by the read/write methods
                    if value.endswith(".json"):
                        value = value.replace(".json", "")
                    logger.debug(f"{utils.get_function_name()}: Before: {class_var} = {class_vars[class_var]}")
                    setattr(cls, class_var, value)
                    new_vars = vars(cls)
                    logger.debug(f"{utils.get_function_name()}: After: {class_var} = {new_vars[class_var]}")
                else:
                    logger.error(
                        f"{utils.get_function_name()}: Invalid value ({value}) for argument ({param}). "
                        + f"Allowed values are [{allowed_parameters[param]}]."
                    )

        if not cls._GAME_DATA:
            logger.info("{utils.get_function_name()}: Loading game data from DataBuilder.")
            if not DataBuilder.is_initialized:
                if not DataBuilder.initialize():
                    logger.error(f"A{utils.get_function_name()}: n error occurred while initializing DataBuilder.")
                    return False
            try:
            ...
            logger.info("{utils.get_function_name()}: Game stat information provided.")
        ...
        try:
            ...
            logger.error(f"{utils.get_function_name()}: Unable to initialize stat data structures. [{key_err_str}]")
            raise StatCalcRuntimeError(
                f"Unable to initialize stat data structures. [{key_err_str}]"
            )

        logger.info(f"{utils.get_function_name()}: Loading unit 'nameKey' mapping object from DataBuilder files...")
        try:
            cls._UNIT_NAME_MAP = DataBuilder.get_unit_names_mapping(cls._LANGUAGE)
        except FileNotFoundError:
            logger.error(f"{utils.get_function_name()}: Unit name mapping file not found. [{cls._LANGUAGE}]")
            logger.info(
                f"{utils.get_function_name()}: Creating unit 'nameKey' mapping object from game server data."
            )
            cls._load_unit_name_map()

        logger.info(
            f"{utils.get_function_name()}: Loading stat ID to localized name mapping for language: {cls._LANGUAGE} ..."
        )
        if not cls._load_stat_name_map():
            logger.error(f"{utils.get_function_name()}: Unable to load stat ID to name mapping. "
                         + f"Stats will be referenced by ID only.")
        else:
            logger.info(f"{utils.get_function_name()}: Stat ID to localized name mapping loading complete.")

        logger.debug("{utils.get_function_name()}: Setting StatCalc._INITIALIZED to True")

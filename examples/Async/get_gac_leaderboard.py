...
Sample script to get either the GAC bracket or league/division leaderboards using the async Comlink client
"""
...
async def main():
    ...
    async with SwgohComlinkAsync() as comlink:
        """
        The get_gac_leaderboard() method takes several required arguments. The combination of arguments required varies
        depending on the GAC leaderboard desired. The parameters are:
        ...
        for player in lb_entries:
            print("Player nane: {name:{name_width}} Guild: {g_name:{g_width}} Skill Rating: {rating}".format(
                name=player[0],
